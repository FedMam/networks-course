...
blacklist_filename = current_directory + '/blacklist.config'
...
def fetch_url(url: str):
    for bl_line in blacklist:
        if url.startswith(bl_line):
            journal.write(f'{datetime.datetime.now().isoformat()} [WARN] Forbidden: {url}\n')
            journal.flush()
            return Response('{"error": "' + bl_line + ' is a blacklisted site"}',
                            status=403,
                            content_type='application/json')
...
if __name__ == '__main__':
    ...
    try:
        with open(blacklist_filename, 'r') as bl_file:
            bl_lines = list(map(lambda it: it.strip(), bl_file.readlines()))
            blacklist = bl_lines + list(map(lambda it: 'www.' + it, bl_lines))
    except OSError as E:
        print('Error: cannot open blacklist.config file')
        exit(-1)
