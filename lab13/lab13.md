...
#### Демонстрация работы
Код программы в файле `crc_test.py`. Логика CRC-алгоритма реализована в файле `crc.py`. Программа `crc_test.py` запускается в двух режимах: основном (`crc_test.py main`) и тестовом (`crc_test.py test`). В основном режиме нужно указать имя файла (одного из файлов в папке `files`), и программа будет симулировать процесс их пересылки по ненадёжному каналу, где каждый пакет имеет 10% вероятности быть повреждённым (в нём инвертируются несколько бит). Каждый пакет состоит из 5 байт документа и одного байта контрольной суммы (который также может быть повреждён). Контрольная сумма как вычисляется, так и проверяется по алгоритму CRC-8.

Скриншот процесса пересылки:
<img src="screenshots/00_crc.png"/>
...
#### Демонстрация работы
В файле `sniffer.py` реализованы пункт А и пункт Б. Скрин с демонстрацией работы в пункте Б.
...
#### Демонстрация работы
В файле `sniffer.py` реализованы пункт А и пункт Б. Приложение нужно запускать как root-пользователь.
<img src="screenshots/01_sniffer.png"/>
