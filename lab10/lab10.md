...
<img src="screenshots/00_ws_ip_first.png"/>
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Identification, Header Checksum, TTL (но не всегда)
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length, DSF, Total Length, Flags, Fragment Offset, Protocol, Source Address, Destination Address
      - Version, Header Length, DSF, Protocol
      - Total Length, Flags, Fragment Offset, Source Address, Destination Address
...
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   ...
   - `0x846d (33901)`
   - 1
   <img src="screenshots/01_ws_ip_example.png"/>
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Да, они сохраняются в ICMP-датаграмме вместе со всем IP-заголовком; но значение Identification в IP-датаграмме, содержащей ICMP-датаграмму, и значение Identification в IP-датаграмме, на которую ссылается внутренняя ICMP-датаграмма, различаются.
   <img src="screenshots/02_ws_ip_closest.png"/>
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - На скрине выше: `0x22b1 (8881)`
   - 64
   
   Если имеется в виду значения полей в внутреннем куске IP-датаграммы, то `0x846f (33903)` и 1
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   <img src="screenshots/03_ws_fragmented.png"/>
