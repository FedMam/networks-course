...
_Скриншот к заданию X находится в файле netX.png._
...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Русский (ru-RU, ru), английский (en-US, en), французский (fr), испанский (es), немецкий (de), греческий (el), китайский (zh-CN, zh)
   - тип данных в ответе (html, application/xml, изображения и другие); способ кодирования/сжатия
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - В информации об ответе в секции Line-based text data можно найти полный код HTML. Также его можно найти в байтовом представлении ответа.
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - 1 (не считая запроса к сайту favicon.ico, видимо, чтобы получить значок сайта)
   - 36
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 (не считая запроса к favicon.ico)
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Я думаю, они были загружены параллельно, так как номера запросов GET и ответов на них в одном запуске эксперимента перекрывались, то есть второй запрос был отправлен до того, как первый получил ответ. Номера: запрос 53 - ответ 62; запрос 59 - ответ 65.
